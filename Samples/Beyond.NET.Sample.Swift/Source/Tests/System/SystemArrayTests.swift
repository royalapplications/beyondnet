...
final class SystemArrayTests: XCTestCase {
	...
	func testSystemArrayConvertedToIList() {
		...
		guard let iList = DNObjectCastTo(arrayOfDateTime,
		...
		defer { System_Collections_IList_Destroy(iList) }
		
		let index: Int32 = 0
		
		System_Collections_IList_Item_Set(iList,
										  index,
										  now,
										  &exception)
		...
		guard let retrievedNow = System_Collections_IList_Item_Get(iList,
																   index,
																   &exception),
			  exception == nil else {
